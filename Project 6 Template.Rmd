...
#The outcome is a vector which counts the total number of people by treatment and covarriate value
rand_fun = function(){
  rand_vec =  sample(0:1,nrow(ypsps),rep=TRUE,prob=c(0.5,0.5))
  dat = ypsps%>% select(parent_Vote)%>%mutate(sim=rand_vec)
...
\textbf{Your Answer} first draft: We see that the covariate we chose to explore - parent_vote - isn't balanced for treatment and control. 
In the real expriment, the control group is underrepresented in the group of students whose parents voted. 
In randomized assignment, we would expect treatment and control to have similar distributions covarriates.
This is a good example for how the independence of treatment assignment and baseline covariates does not
guarantee balance of treatment assignment and baseline covariates. 
This imbalance raises concerns to whether this covariate and parhaps other covariates are confounders.
...
ypsps_selected = ypsps %>% select(college,parent_Vote,parent_CivicOrg,parent_OwnHome
                                  ,parent_Rally,parent_PolClub,parent_Money
                                  ,student_AdultTalk,student_FrTalk
                                  ,student_Newspaper,student_Magazine
                                  ,student_Radio,student_FamTalk
                                  )
...
balance_fun = function(dat,n){
  dat_sub = dat%>% select(college,colnames(dat)[n])
  result = data_frame(covariate = as.character(colnames(dat)[n])
                      ,control = round(mean((dat_sub%>%filter(college==0)%>% select(-college))[,1]),3)
                     ,treatment = round(mean((dat_sub%>%filter(college==1)%>% select(-college))[,1]),3)
...
ypsps_pre_treat = ypsps%>%select(!matches("1973")& !matches("1982") 
                                 & -c("student_ppnscal","college","student_vote"
                                     ,"student_meeting","student_button"
...
# model_ps <- glm(college ~ parent_Vote + parent_CivicOrg + parent_OwnHome
#                 ,family = binomial()
#                 ,data = ypsps_selected)
# 
# dat <- ypsps_selected %>% mutate(prop_score = predict(model_ps))
# 
# #subset dataset
# df_a0 <- dat %>% filter(college == 0)
# df_a1 <- dat %>% filter(college == 1)
# 
# dist.prop.score <- function(x,y) { abs(x-y) }
# 
# calculate.dist <- function(x, y, dist.method, xnames = df_a1$ID, ynames = df_a0$ID) {
...
# }
# 
# dists_ps <- calculate.dist(as.matrix(df_a1[, "prop_score"]),
#                            as.matrix(df_a0[, "prop_score"]),
#                            dist.prop.score)
# as.data.frame(dists_ps)
