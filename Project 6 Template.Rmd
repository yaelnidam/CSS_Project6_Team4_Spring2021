...
sims=100
#create randomized treatment/control columns for each simulation
...
#create empty lists to contain the sum of students whose parents voted by treatment and control for each simulation
treatment = c()
control = c()

#Run the simulation and store values in lists
for (i in 1:sims){
  
    dat=rand_treat%>% select(paste0("sim",i),"parent_Vote")
    colnames(dat)=c("sim","parent_Vote")
    tot = dat%>% group_by(sim)%>%
      summarise(sum_parent_Vote=sum(parent_Vote),.groups = 'drop')

    treatment[i]=as.integer(tot[2,2])
    control[i]=as.integer(tot[1,2])
}

#calculate the mean number of people whose parents voted by randomized treatment/control
mean(treatment)
mean(control)
```
