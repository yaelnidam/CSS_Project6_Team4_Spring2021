...
In the real experiment, the control group is underrepresented in the group of students whose parents voted. 
...
ypsps_selected = ypsps %>% dplyr::select(college,parent_Vote,parent_CivicOrg,parent_OwnHome
...
ggplot(dt, aes(x=p_balanced_cov, y=ATT)) +
  geom_point() +
  geom_line(aes(x=.17), linetype = "dotted", color="red") +
...
```
    \item \textbf{Your Answer:} The alternative model which uses Malahanobis exact matching has less variation in mean improvement and significantly more positive cases of improvement. The propensity score models actually worsen balance.
\end{enumerate}
