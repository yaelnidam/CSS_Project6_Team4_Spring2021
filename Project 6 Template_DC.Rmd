...
student_Gen, student_GPA, student_NextSch, parent_Employ, parent_EducHH, parent_EducW
parent_HHInc, parent_OwnHome. 

We see that none of our covariates are balanced, thus the proportion of covariates meeting the balance threshold is equal to 0.
...
```{r}
dt %>%
  subset(p_balanced_cov>0.15) %>%
  nrow()
```
    \item \textbf{Your Answer}: A total of 99 out of 100 simulations [this will change when we run a larger number of sims] resulted in models with a higher proportion of balanced covariates. However, as we see in the plot, there appears to be a greater amount of variation in the Average Treatment Effect on the Treated, which suggests that our method for matching is imprecise.
...
ggplot(data = dt, aes(x=ATT)) +
  ...
  ylab("Count")
```
    \item \textbf{Your Answer:} As we mentioned above, in the plot showing the distribution of the Average Treatment Effect on the Treated, we identify outlier values that skew the distribution to the right. Again, we do have concerns about the precision of our model, given the variation in treatment effect. 
...
dt_alt %>%
  ...
  nrow()
```
    \item \textbf{Your Answer:} No. [But maybe this changes when we run a larger number of simulations].
...
```
    \item \textbf{Your Answer:} The alternative model, which uses Mahalanobis exact matching, has less variation in mean improvement and significantly more positive cases of improvement. We can see that the propensity score models actually worsen balance.
\end{enumerate}
